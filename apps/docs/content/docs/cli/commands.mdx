...
Launch a React-based web interface for visual agent testing and management. By default, opens the hosted production interface at [adk-web.iqai.com](https://adk-web.iqai.com).
...
# Use local development web application
adk web --local
# Use local development web application on specific port
...
- `--host <host>` – Host for API server (default: localhost). Use 0.0.0.0 to make accessible from other devices
- `--dir <directory>` – Directory to scan for agents (default: current directory)
- `--local` – Use local development application instead of opening production URL
- `--web-port <port>` – Port where local adk-web app is running (default: 3000)
...
- **Local Development (`--local`)**: Use when you've downloaded and are running the adk-web code locally. Specify the port with `--web-port` if not running on default port 3000
