...
Core metrics and scoring algorithms are now available in `@iqai/adk` as part of the experimental evaluation framework. Manual evaluation can still supplement automated metrics.
</Callout>
...
**ROUGE Metrics (Available):**
- **ROUGE-1**: Unigram overlap between agent response and reference (F-measure)
- **Implementation**: Available in `ResponseEvaluator` class
- **Threshold**: Typically 0.7+ for production quality
- **Usage**: Automatic similarity scoring between expected and actual responses

```typescript
import { ResponseEvaluator } from '@iqai/adk/evaluation';

const evaluator = new ResponseEvaluator();
const result = await evaluator.evaluateInvocations(invocations);
console.log(`ROUGE Score: ${result.overallScore}`);
```
...
1. **TOOL_TRAJECTORY_AVG_SCORE**:
   - Exact matching of tool call sequences
   - Binary scoring (1.0 for perfect match, 0.0 for any mismatch)
   - Validates tool names, arguments, and execution order

2. **RESPONSE_MATCH_SCORE**:
   - ROUGE-1 F-measure for text similarity
   - Compares actual vs expected response content
   - Range: 0.0 to 1.0

3. **RESPONSE_EVALUATION_SCORE**:
   - LLM-as-judge evaluation for response quality
   - Scale: 1-5 with configurable judge models
   - Assesses helpfulness, accuracy, and appropriateness

4. **SAFETY_V1**:
   - Safety and harmlessness evaluation
   - Integration with Vertex AI safety models
