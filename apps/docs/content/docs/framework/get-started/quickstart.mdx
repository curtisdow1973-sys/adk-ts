...
Create a simple agent in `src/agent.ts`:
...
Now let's test your agent. Create a simple script to interact with it. Add the following to `src/main.ts`:
...
Replace your `src/agent.ts` with this enhanced version:
...
For a command-line chat experience:
...
For a more visual chat experience with a web interface:
