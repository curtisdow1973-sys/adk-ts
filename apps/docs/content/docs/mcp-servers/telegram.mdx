...
<Tabs items={['Simple', 'Verbose', 'Claude Desktop']}>
  <Tab value="Simple">
    ### Using McpTelegram Wrapper

    The simplest way to use MCP Telegram with ADK TypeScript:

    ```typescript
    import { McpTelegram } from "@iqai/adk";

    // Create Telegram toolset with simplified syntax
    ...
    console.log(`Retrieved ${mcpTools.length} Telegram tools`);
    ```
  </Tab>
  ...
  <Tab value="Claude Desktop">
    ...
    Then restart Claude Desktop and you'll have access to Telegram tools directly in your conversations.

    **Example prompts:**
    - "Send a message to @mychannel saying 'Hello from Claude!'"
    - "Get information about channel @mychannel"
    - "Forward message 123 from @channel1 to @channel2"
    - "Pin message 456 in @mychannel"
    - "Get the administrators of @mychannel"
  </Tab>
</Tabs>
