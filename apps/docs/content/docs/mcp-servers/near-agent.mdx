...
<Tabs items={['Simple', 'Verbose', 'Claude Desktop']}>
  <Tab value="Simple">
    ### Using McpNearAgent Wrapper

    The simplest way to use MCP NEAR Agent with ADK TypeScript:

    ```typescript
    import { McpNearAgent } from "@iqai/adk";

    // Create NEAR Agent toolset with simplified syntax
    ...
    console.log(`Retrieved ${mcpTools.length} NEAR Agent tools`);
  ...
  <Tab value="Claude Desktop">
    ...
    **For Mainnet:**
    ...
    Then restart Claude Desktop and you'll have access to NEAR blockchain tools directly in your conversations.

    **Example prompts:**
    - "Check my NEAR account balance"
    - "Transfer 2 NEAR tokens to bob.testnet"
    - "What's my account information?"
    - "Call the get_greeting method on contract.testnet"
    - "Show me my recent transaction history"
    - "Get information about the current network validators"
  </Tab>
</Tabs>
