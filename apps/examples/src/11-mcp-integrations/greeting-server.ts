...
async function main() {
	...
	server.addTool({
		name: "greet_user",
		description: "Greets a user by asking for their name through sampling for personalization",
		parameters: z.object({}),
				...
				if (context && typeof context === 'object' && 'session' in context) {
					...
					if (session && typeof session.requestSampling === 'function') {
						...
						const samplingResponse = await session.requestSampling({
							...
							systemPrompt: "You are a helpful assistant that provides user context information. Respond concisely with just the requested information.",
						...
						let userName = "there";
						if (samplingResponse && samplingResponse.content && samplingResponse.content.length > 0) {
	...
	server.addTool({
		...
		parameters: z.object({
			operation: z.string().describe("The mathematical operation to perform (e.g., '25 * 8', '100 / 4')"),
		}),
	...
	server.addTool({
		name: "server_info",
		description: "Provides information about this MCP server and its capabilities",
		parameters: z.object({}),
		execute: async () => {
			return JSON.stringify({
				...
				description: "A demonstration MCP server using FastMCP showing sampling concepts and basic tools"
			}, null, 2);
		},
	});
