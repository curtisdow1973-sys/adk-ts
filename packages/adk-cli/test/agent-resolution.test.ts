...
describe('ADKServer agent resolution', () => {
  ...
  it('fails on directory missing agent file', async () => {
    ...
    (server as any).agents.set('empty', { relativePath: 'empty', name: 'empty', absolutePath: tmp });
    await expect(load(server, 'empty')).rejects.toThrow(/No agent\.js or agent\.ts/);
  });
  ...
  it('scanAgents updates when new agent appears', async () => {
    ...
    ;(server as any).agents.set('dynamic', { relativePath: 'dynamic', name: 'dynamic', absolutePath: dynamicDir });
