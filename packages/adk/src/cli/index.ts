...
import { toCloudRun } from "./cli-deploy";
...
import { HelpfulCommand } from "./helpful-commands";
...
program.addCommand(
	new HelpfulCommand("create")
		...
		.option(
			"--api_key <apiKey>",
			"Optional. The API Key needed to access the model.",
		)
		.option(
			"--project <project>",
...
program.addCommand(
	new HelpfulCommand("run")
		...
		.option(
			"--replay <replayFile>",
			"Path to a JSON file with initial state and user queries. Creates a new session with this state and runs the queries without interactive mode.",
		)
		.option(
			"--resume <resumeFile>",
			"Path to a previously saved session file. Replays the session and continues in interactive mode.",
		)
		.action(async (agentFolderName, options) => {
			...
			const agentParentDir =
				options.parentDir || process.env.INIT_CWD || process.cwd();
			...
			try {
				...
				console.warn(
					'Graphviz is not installed. The "graph" command will not work.',
				);
			}
			...
			try {
				...
				if (options.replay && options.resume) {
					console.error(
						"Error: The --replay and --resume options cannot be used together.",
					);
					process.exit(1);
				}
				...
				try {
					...
					console.warn(
						"Failed to register ts-node. If you have TypeScript files, this might cause issues.",
					);
				}
				...
				if (agentFolderName === ".") {
					...
					console.log(
						`Running agent with parent dir: ${agentParentDir}, folder name: ${agentFolderName}`,
					);
...
program
	...
	.option(
		"--output <outputFile>",
		'Path to save the graph image. Default is "<agent_name>_graph.png"',
	)
	.option(
		"--highlight <pairs...>",
		'Pairs of node names to highlight in the graph, e.g., "agent1,agent2"',
		[],
	)
	.action(async (agent: string, options: any) => {
		try {
			...
			const agentModulePath = path.resolve(
				process.cwd(),
				agentParentDir,
				agentFolderName,
				"index.ts",
...
program
	...
	.option(
		...
		"The service name to use in Cloud Run.",
	...
	.option(
		"--trace_to_cloud",
		"Whether to enable Cloud Trace for cloud run.",
		false,
	)
	.option("--with_ui", "Deploy ADK Web UI if set.", false)
	.option(
		"--temp_folder <tempFolder>",
		"Temp folder for the generated Cloud Run source files.",
	)
	.option(
		"--verbosity <verbosity>",
...
program
	.command("web [agent]")
	.description(
		"Starts a web server for agents with Socket.IO for live interaction.",
	)
